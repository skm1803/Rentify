// curl -X 'POST' \
//   'http://127.0.0.1:8000/v1/properties/filter' \
...
export default function PropertiesBySeller() {
    const [properties, setProperties] = useState([]);
    const { id } = useParams();

    useEffect(() => {
        axios.post(`http://127.0.0.1:8000/v1/properties/filter`, {seller_id: id})
            .then(response => {
    ...
    return (
        <div>
            <div className="flex justify-center">
                <h1 className="text-3xl font-bold">Properties by Seller</h1>
            </div>
